...
while($row = $result->fetch_assoc()) {
    ...
    $sale_mail = $row['sales_email'];
		...
		<input type="text" value="https://webappsolution.org/payment_us/paynow.php?token=<?= $link_token ?>" id="myInput" style="opacity: 0;"><br>
		...
		<a class="btn btn-primary mb-3"  href="https://webappsolution.org/payment_us/paynow.php?token=<?= $link_token ?>">Complete This Invoice</a>
		<button class="btn btn-info  mb-3" id="replaceit" onclick="copyIt()"  href="https://webappsolution.org/payment_us/paynow.php?token=<?= $link_token ?>">Copy to clipboard</button>
		<script>
		    function copyIt() {
              /* Get the text field */
...
<?php } else{
 header('Location: https://webappsolution.org/payment_us');
} ?>